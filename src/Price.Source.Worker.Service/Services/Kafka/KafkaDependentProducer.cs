...
namespace Price.Source.Worker.Service.Services.Kafka
{
    ...
    public class KafkaDependentProducer<K, V> : IKafkaDependentProducer<K, V>
    {
        ...
        public KafkaDependentProducer(IKafkaClientHandle handle, IAsyncSerializer<V> serializer)
        {
            var builder = new DependentProducerBuilder<K, V>(handle.Handle);
            builder.SetValueSerializer(serializer);
            kafkaHandle = builder.Build();
        }
